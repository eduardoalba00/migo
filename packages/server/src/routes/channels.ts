...
export function channelRoutes(
  ...
  return async function (app: FastifyInstance) {
    ...
    app.post(
      fastifyRoute(SERVER_ROUTES.CATEGORIES_CREATE),
      ...
      async (request, reply) => {
        ...
        if (!isOwner) {
          return reply
            .status(403)
            .send({ error: "Only the server owner can create categories" });
        }
    ...
    app.delete(
      fastifyRoute(SERVER_ROUTES.CATEGORIES_DELETE),
      { preHandler: requireAuth },
      async (request, reply) => {
        const { serverId, categoryId } = request.params as {
          serverId: string;
          categoryId: string;
        };
        ...
        if (!isOwner) {
          return reply
            .status(403)
            .send({ error: "Only the server owner can delete categories" });
        }
    ...
    app.post(
      fastifyRoute(SERVER_ROUTES.CHANNELS_CREATE),
      { preHandler: requireAuth },
      async (request, reply) => {
        const { serverId } = request.params as { serverId: string };
        ...
        if (!isOwner) {
          return reply
            .status(403)
            .send({ error: "Only the server owner can create channels" });
        }
        ...
        if (parsed.data.categoryId) {
          ...
          if (!cat) {
            return reply
              .status(400)
              .send({ error: "Category not found in this server" });
          }
        }
    ...
    app.get(
      fastifyRoute(SERVER_ROUTES.CHANNELS_LIST),
      { preHandler: requireAuth },
      async (request, reply) => {
        const { serverId } = request.params as { serverId: string };
    ...
    app.patch(
      fastifyRoute(SERVER_ROUTES.CHANNELS_UPDATE),
      { preHandler: requireAuth },
      async (request, reply) => {
        const { serverId, channelId } = request.params as {
          serverId: string;
          channelId: string;
        };
        ...
        if (!isOwner) {
          return reply
            .status(403)
            .send({ error: "Only the server owner can update channels" });
        }
    ...
    app.delete(
      fastifyRoute(SERVER_ROUTES.CHANNELS_DELETE),
      { preHandler: requireAuth },
      async (request, reply) => {
        const { serverId, channelId } = request.params as {
          serverId: string;
        ...
        if (!isOwner) {
          return reply
            .status(403)
            .send({ error: "Only the server owner can delete channels" });
        }
    ...
    app.post(
      fastifyRoute(READ_STATE_ROUTES.ACK),
      { preHandler: requireAuth },
      async (request, reply) => {
        ...
        const existing = await db
          ...
          .where(
            and(
              eq(readStates.userId, request.user.sub),
