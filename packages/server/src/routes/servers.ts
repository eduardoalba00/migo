...
export function serverRoutes(
  ...
  return async function (app: FastifyInstance) {
    ...
    app.post(
      fastifyRoute(SERVER_ROUTES.CREATE),
    ...
    app.get(
      fastifyRoute(SERVER_ROUTES.LIST),
      { preHandler: requireAuth },
      async (request, reply) => {
    ...
    app.get(
      fastifyRoute(SERVER_ROUTES.GET),
      { preHandler: requireAuth },
      async (request, reply) => {
        const { serverId } = request.params as { serverId: string };

        const isMember = await serverService.isMember(
          serverId,
          request.user.sub,
        );
        if (!isMember) {
          return reply
            .status(403)
            .send({ error: "Not a member of this server" });
        }
    ...
    app.patch(
      fastifyRoute(SERVER_ROUTES.UPDATE),
      { preHandler: requireAuth },
      async (request, reply) => {
        const { serverId } = request.params as { serverId: string };
        ...
        if (!isOwner) {
          return reply
            .status(403)
            .send({ error: "Only the server owner can update the server" });
        }
    ...
    app.delete(
      fastifyRoute(SERVER_ROUTES.DELETE),
      { preHandler: requireAuth },
      async (request, reply) => {
        const { serverId } = request.params as { serverId: string };
        ...
        if (!isOwner) {
          return reply
            .status(403)
            .send({ error: "Only the server owner can delete the server" });
        }
    ...
    app.get(
      fastifyRoute(SERVER_ROUTES.MEMBERS),
      { preHandler: requireAuth },
      async (request, reply) => {
        const { serverId } = request.params as { serverId: string };
    ...
    app.delete(
      fastifyRoute(SERVER_ROUTES.LEAVE),
      { preHandler: requireAuth },
      async (request, reply) => {
        const { serverId } = request.params as { serverId: string };
        ...
        await db.delete(serverMembers).where(eq(serverMembers.id, member.id));
    ...
    app.delete(
      fastifyRoute(SERVER_ROUTES.KICK_MEMBER),
      { preHandler: requireAuth },
      async (request, reply) => {
        const { serverId, userId } = request.params as {
          serverId: string;
          userId: string;
        };
        ...
        if (!isOwner) {
          return reply
            .status(403)
            .send({ error: "Only the server owner can kick members" });
        }
    ...
    app.put(
      fastifyRoute(SERVER_ROUTES.BAN_CREATE),
      { preHandler: requireAuth },
      async (request, reply) => {
        const { serverId, userId } = request.params as {
          serverId: string;
        ...
        if (!isOwner) {
          return reply
            .status(403)
            .send({ error: "Only the server owner can ban members" });
        }
    ...
    app.delete(
      fastifyRoute(SERVER_ROUTES.BAN_DELETE),
      { preHandler: requireAuth },
      async (request, reply) => {
        const { serverId, userId } = request.params as {
          serverId: string;
        ...
        if (!isOwner) {
          return reply
            .status(403)
            .send({ error: "Only the server owner can unban members" });
        }
    ...
    app.get(
      fastifyRoute(SERVER_ROUTES.BANS_LIST),
      { preHandler: requireAuth },
      async (request, reply) => {
        const { serverId } = request.params as { serverId: string };
        ...
        if (!isOwner) {
          return reply
            .status(403)
            .send({ error: "Only the server owner can view bans" });
        }
